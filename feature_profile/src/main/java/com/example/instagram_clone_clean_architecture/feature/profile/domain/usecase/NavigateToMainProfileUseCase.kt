...
class NavigateToMainProfileUseCase(
    private val navigationManager: NavigationManager,
    defaultDispatcher: CoroutineDispatcher = Dispatchers.Main
) : UseCase<Unit, NavigateToMainProfileUseCase.Param>(defaultDispatcher) {

    override suspend fun run(params: Param): Either<Unit, Failure> {
        navigationManager.onNavEvent(params.navDir)
        return Either.Success(Unit)
    }

    data class Param(val navDir: NavDirections)
}